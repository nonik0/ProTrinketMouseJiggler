// adapted from code here: https://learn.sparkfun.com/tutorials/tech-prank-hardware-mouse-jiggler
// to work with Adafuit Pro Trinket USB mouse library: https://learn.adafruit.com/pro-trinket-usb-hid-mouse/overview
...
void mouseJump() {
  TrinketMouse.move(random(JUMP_MIN, JUMP_MAX), random(JUMP_MIN, JUMP_MAX), 0, 0);
}
...
void mouseLine() {
  ...
  for ( int j = 0; j < line_num; j++ ) {
    ...
    TrinketMouse.move((int)(x_acc + 0.5), (int)(y_acc + 0.5), 0, 0);
...
void mouseJitter() {
  ...
  for ( int i = 0; i < jitter_times; i++ ) {
    TrinketMouse.move(random(JITTER_MIN, JITTER_MAX), random(JITTER_MIN, JITTER_MAX), 0, 0);
    delay(10);
  }
}
